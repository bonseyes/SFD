...
}""" % (name, name, name, float(min_box), max_box_str, aspect_ratio_str, step))
...
"""layer {
    ...
    def generate(self, stage, gen_ssd, size, class_num):
      ...
      self.stage = stage
      if(stage == "train"):
          self.file = "train.prototxt"
      elif(stage == "test"):
          self.file = "test.prototxt"
      else:
          self.file = "deploy.prototxt"
      ...
      if gen_ssd is True:
          ...
          self.concat_boxes(['conv3', 'conv5', 'conv11', 'conv13', 'conv14_2', 'conv15_2'])
          if stage == "train":
             self.ssd_loss()
          elif stage == "deploy":
             self.ssd_predict()
