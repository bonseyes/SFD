...
layer{
  ...
  loss_weight: 1 
    ...
    def ssd_predict(self):
      print(
"""layer {
  name: "mbox_conf__reshape"
  type: "Reshape"
  bottom: "mbox_conf_"
  top: "mbox_conf__reshape"
  reshape_param {
    shape {
      dim: 0
      dim: -1
...
layer {
  name: "mbox_conf__softmax"
  type: "Softmax"
  bottom: "mbox_conf__reshape"
  top: "mbox_conf__softmax"
  softmax_param {
    axis: 2
  }
}
layer {
  name: "mbox_conf__flatten"
  type: "Flatten"
  bottom: "mbox_conf__softmax"
  top: "mbox_conf__flatten"
  flatten_param {
    axis: 1
  }
}
...
"""layer {
  ...
  concat_param {
    ...
    def conv(self, name, out, kernel, stride=1, group=1, bias=False, bottom=None, mbox=False):

      if self.stage == "deploy" and mbox == False: #for deploy, merge bn to bias, so bias must be true
          #bias = True
          bias = False
...
"""layer {
  ...
  top: "%s"
    ...
    def conv_dw_pw(self, name, inp, outp, stride):
      ...
      if self.quantized == False: # quantized model remove these layers after depthwise conv layers 
          self.bn(name1)
          self.relu(name1)
      name2 = name 
...
"""layer {
  ...
  flatten_param {
    ...
    def mbox_prior(self, name, min_size, max_size, step, aspect_ratio):
      min_box = min_size # will pass the min_size list directly
...
"""layer {
  ...
  prior_box_param {%s%s%s
    ...
    def mbox_conf(self, bottom, num):
       ...
       self.conv(name, num, 1, bias=True, bottom=bottom, mbox=True)
       self.permute(name)
       self.flatten(name)
...
"""layer {
    ...
    def generate(self, stage, gen_ssd, size, class_num, quantization):
      self.class_num = class_num
      self.lmdb = FLAGS.lmdb
      self.label_map = FLAGS.label_map
      self.stage = stage
      self.quantized = quantization
      ...
      if gen_ssd:
          ...
          self.header("MobileNet-classifier")
      ...
      if gen_ssd is True:
          ...
          self.mbox("conv11", 1)
          self.mbox("conv13", 1)
          self.mbox("conv14_2", 1)
          self.mbox("conv15_2", 1)
          self.concat_boxes(['conv5', 'conv11', 'conv13', 'conv14_2', 'conv15_2'])
          if stage == "train":
             self.ssd_loss()
          elif stage == "deploy":
             self.ssd_predict()
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--quantization',
      action='store_true',
      help='Default generate normal ssd, if this is set, generate quantized version of MobileNetV1'
  )
  FLAGS, unparsed = parser.parse_known_args()
  gen = Generator()
  gen.generate(FLAGS.stage, not FLAGS.classifier, FLAGS.size, FLAGS.class_num, FLAGS.quantization)
