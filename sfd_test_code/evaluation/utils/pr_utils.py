...
def compute_prec_rec(det_faces, gt_faces, gt_keep):
    ...
    respectively.
    """
    total_imgs = np.sum([len(gt_faces[e]) for e in gt_faces])
    all_interp_pr = np.zeros((total_imgs, THRESHOLDS, 2), dtype=np.float32)
    count = 0
    img_idx = -1 
    for event in det_faces:
        for img in det_faces[event]:
            ...
            all_interp_pr[img_idx, :, :] = interp_pr

    tpfp = np.zeros(THRESHOLDS)
    tp = np.zeros(THRESHOLDS)
    for elem in all_interp_pr:
        tpfp = tpfp + elem[:, 0]
        tp = tp + elem[:, 1]
