...
if __name__ == '__main__':
    ...
    for index, event in enumerate(event_list):
        ...
        for num, file in enumerate(filelist):
            ...
            ## Lets perform a resizing of the orig image to avoid memory errors
            pixels = 531 * 1024
            x_shrink = 1
            if (image.shape[0] * image.shape[1]) > pixels:
                x_shrink = pixels / float(image.shape[0] * image.shape[1])

            max_im_shrink = (0x7fffffff / 4000.0 / (image.shape[0] * image.shape[1])) ** 0.5 # the max size of input image for caffe
            max_im_shrink *= x_shrink
