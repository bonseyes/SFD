...
"""layer {
  ...
  type: "Input"
  top: "data"
  input_param { shape: { dim: 1 dim: 3 dim: %d dim: %d } }
...
"""layer {
    ...
    top: "label"
    image_data_param {
        source: "/media/sdf/IMAGENET/train_shuffled.txt"
        batch_size: 40
        new_height: 256
        new_width: 256
    }
...
layer {
  ...
  top: "label"
  image_data_param {
      source: "/media/sdf/IMAGENET/val.txt"
      batch_size: 10
      new_height: 256
      new_width: 256
  }
...
"""layer {
    ...
    def generate(self, stage, gen_ssd, size, class_num, nobn, eps, relu6):
      ...
      t = 2
      t_ = 4
      self.conv_bn_relu_with_factor("Conv", 6, 3, 2)
      self.conv_bn_relu_with_factor("Conv_a", 6, 3, 1)
      self.conv_depthwise("conv/depthwise", 6, 1)
      self.conv_project("conv/project", 6, 6)
      self.conv_block("conv_1", 6,  t, 12 , 2, False) #
      self.conv_block("conv_2", 12,  t, 12, 1, True)
      self.conv_block("conv_3", 12,  t, 12, 1, True)  # 
      self.conv_block("conv_4", 12,  t, 12, 1, True)
      self.conv_block("conv_5", 12,  t, 24, 2, False) #
      self.conv_block("conv_6", 24,  t, 24, 1, True)
      self.conv_block("conv_7", 24,  t, 24, 1, True)
      self.conv_block("conv_8", 24,  t, 24, 1, True) #
      self.conv_block("conv_9", 24,  t, 24, 1, True)
      self.conv_block("conv_10", 24,  t, 48, 2, False) # 
      self.conv_block("conv_11", 48,  t, 48, 1, True)
      self.conv_block("conv_12", 48,  t, 48, 1, True)
      self.conv_block("conv_13", 48,  t, 48, 1, True)
      self.conv_block("conv_14", 48,  t, 48, 1, True)
      self.conv_block("conv_15", 48,  t, 48, 1, True)
      self.conv_block("conv_16", 48,  t, 48, 1, True)
      self.conv_bn_relu_with_factor("Conv_1", 96, 1, 1)
      if gen_ssd is True:
          self.conv_ssd("layer_19", 2, 96, 128)
          self.conv_ssd("layer_19", 3, 128, 171)
          self.conv_ssd("layer_19", 4, 171, 214)
