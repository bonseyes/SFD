...
bool MeetEmitConstraint(const NormalizedBBox& src_bbox,
  ...
  if (emit_type == EmitConstraint_EmitType_CENTER) {
	...
	(src_bbox.xmax() - src_bbox.xmin()) >= 0.009375 &&
	(src_bbox.ymax() - src_bbox.ymin()) >= 0.009375) { // bbox > 6 pixels (6/640 = 0.009375)
      return true;
    } else {
      return false;
    }
...
void MatchBBox(const vector<NormalizedBBox>& gt_bboxes,
  ...
  for (int i = 0; i < num_pred; ++i) {   // i >> predictions - anchors
    if (ignore_cross_boundary_bbox && IsCrossBoundaryBBox(pred_bboxes[i])) {
      (*match_indices)[i] = -2; // to ignore them
      continue;
    }
    for (int j = 0; j < num_gt; ++j) {  // j >> ground truth
      float overlap = JaccardOverlap(pred_bboxes[i], gt_bboxes[gt_indices[j]]);
      if (overlap > 1e-6) { // to get only pos overlaps
        (*match_overlaps)[i] = std::max((*match_overlaps)[i], overlap); // get only max overlap
        overlaps[i][j] = overlap; // This stores all overlaps
      }
    }
  }
  ...
  while (gt_pool.size() > 0) {
    ...
    if (max_idx == -1) {
      ...
      gt_pool.erase(std::find(gt_pool.begin(), gt_pool.end(), max_gt_idx)); // while goes until size = 0
    }
  }
    ...
    case MultiBoxLossParameter_MatchType_PER_PREDICTION:
    {
      ...
      for (map<int, map<int, float> >::iterator it = overlaps.begin();
        ...
        for (int j = 0; j < num_gt; ++j) {
          ...
          // If no match from previous steops, Match it with any anchor > threshold, but take the max_overlap
     ...
     /*
      // Step 2 in matching startegy
      // First Get indices of GT with less than 6 matches
          // get counts for each element
	  map<int, int> matched_gt_counts;
	  for (int i = 0; i < match_indices->size(); ++i)
	   {
   		   map<int, int>::iterator it(matched_gt_counts.find( (*match_indices)[i]));
   		   if (it != matched_gt_counts.end()){
      		    it->second++;
   		    } 
   		   else {
      		    matched_gt_counts[(*match_indices)[i]] = 1;
   		    }
	   }

	   // Second, update matching boxes for ground truth which have less than 6 matches
	   for (map<int, map<int, float> >::iterator it = overlaps.begin();
           it != overlaps.end(); ++it) {
        int i = it->first;
        if ((*match_indices)[i] != -1) {
        ...
        float max_overlap = -1;
        for (int j = 0; j < num_gt; ++j) {
          if (it->second.find(j) == it->second.end() || matched_gt_counts[j] >= 6) // Number 6 is mentioned in github issues of SFD
           { 
            // No overlap between the i-th prediction and j-th ground truth.
            continue;
          }
          // Find the maximum overlapped pair. 
          float overlap = it->second[j];
          // If no match from previous steps, Match it with any anchor > 0.1
